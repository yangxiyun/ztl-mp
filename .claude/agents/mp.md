...
- 云覆盖 gate（见 routing-table 消歧规则 6）：代账客户资料在 WorkDrive `10_BPS`，云端会话经 zoho-workdrive MCP 直接执行；管理库（ZTL_Manage）上云完成前碰它的任务派回本地；`D:\_BPS\` 已废弃，任何任务包不得指向。数据确实够不着 → MISSING_INPUT 上报，不硬跑。
