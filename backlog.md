...
- [ ] 2026-08-09 【mgmt / 待用户定】**项目执行层与归档层现无落点**：`D:\ZTL_Delivery\`（活跃项目）与 `D:\ZTL_Archive\`（归档）已按用户裁定从路由删除（核实两者在本机根本不存在，属死路径）。相关步骤已改为 GATE_CONFIRM 问落点：`mgmt-convert-to-project` 不再建项目目录、`mgmt-archive-project` 只更新 db 不移动文件、`mgmt-inbox-triage` 三条分诊规则改为留 Inbox。**日后若要恢复这两层，须先定落点**（建议放 `wd:20_ZTLMGMT/` 下作子目录），再改 mgmt 仓那 6 个文件。commit 17de051。
...
- [x] 2026-08-09 【ztl-agent-mgmt】**广能发商机成果回迁云端**（✅ 全部完成：商机 5 件、7 家客户档、`_meta/db` 三表、客户清单.csv 均已与云端 MD5 一致）：`D:\ZTL_Manage\01_商机\S157_广能发老挝EPC项目财税顾问服务_2026.08\`（需求记录/报价函 docx/报价推理/测算报价/服务方案 共 5 件）、`D:\ZTL_Customers\` 下当日新建的 7 份 `_客户信息.md` + `客户清单.csv`、`_meta\db\` 三张表（客户/联系人/商机）当日增量 → 合并进 WorkDrive `20_ZTLMGMT` 对应位置。⚠️ 三张表云端已有 8-09 迁移快照，必须**按行增量合并**，禁整表覆盖。
