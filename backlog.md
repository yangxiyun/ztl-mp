...
- [ ] 2026-08-09 【用户放行 / MP】**云端 `_meta/db` 三表 + 客户清单.csv 更新未完成**：`Z:\20_ZTLMGMT\_meta\db\{客户,联系人,商机}.md` 与 `03_customers\客户清单.csv` 需用本地现版替换（本地 = 云端基线 + 当日增量，已用 MD5 证实云端现版与本地 `.bak-20260809-1510` 逐字节相同，期间无第三方写入，覆盖无损）。云端 `.bak-cloud-20260809` 备份已建好，但覆盖/改名动作被会话权限策略拦截三次，需用户放行相应 Bash 权限或自行拷贝。
- [ ] 2026-08-09 【用户裁定】**另外两个本地根是否也迁云端**：切引用时发现 mgmt 域还依赖 `D:\ZTL_Delivery\`（执行层，活跃项目按业务类型分：01_2026代账业务/02_2026月度报税/03_咨询业务/04_其他/09_质控）与 `D:\ZTL_Archive\`（已完结项目与商机归档）。本轮用户只裁定了 Manage/Customers 两个根，这两个未提及故**原样保留**，仍是纯本地、无云端副本、换机器即够不着。要迁的话可比照本次做（建 `wd:20_ZTLMGMT` 同级团队文件夹或作其子目录，再切 mgmt 仓引用）。
- [x] 2026-08-09 【ztl-agent-mgmt】**切引用到 WorkDrive**（✅ 完成，commit b0beae7 已推送）：11 个文件 37 处硬编码 `D:\ZTL_Manage\` / `D:\ZTL_Customers\`（CLAUDE.md、agents/mgmt-workflow-router、agents/pricing-quote-flow、skills 8 个：mgmt-onboard-opportunity/mgmt-convert-to-project/mgmt-archive-project/mgmt-pipeline-review/mgmt-archive-licenses/mgmt-client-insights/mgmt-inbox-triage/annual-portfolio-rollover）全部改为 `wd:20_ZTLMGMT/...` 记号 + zoho-workdrive MCP 读写。根因：2026-08-09 数据迁云端时**只迁数据没切引用**，当天广能发商机单的全部成果因此落回已废弃的本地旧根。同批把那批成果迁上云（见下一条）。
- [ ] 2026-08-09 【ztl-agent-mgmt】**广能发商机成果回迁云端**（🔄 商机 5 件与 7 家客户档已迁毕，只剩上面那条的三表+csv）：`D:\ZTL_Manage\01_商机\S157_广能发老挝EPC项目财税顾问服务_2026.08\`（需求记录/报价函 docx/报价推理/测算报价/服务方案 共 5 件）、`D:\ZTL_Customers\` 下当日新建的 7 份 `_客户信息.md` + `客户清单.csv`、`_meta\db\` 三张表（客户/联系人/商机）当日增量 → 合并进 WorkDrive `20_ZTLMGMT` 对应位置。⚠️ 三张表云端已有 8-09 迁移快照，必须**按行增量合并**，禁整表覆盖。
