...
- **任务可见**：每接一单在滴答清单「MP派工单」建卡，**格式一律按 `dida-board-contract.md`**——Todo 列 + `agent-ready` + `ws-<代号>` 标签 + `## 任务信息` 描述模板，四项缺一卡片就永远躺着没人认领**且不报错**（2026-08-09 之前该契约不存在，App 侧 MP 建的卡全部因此卡死）。建卡责任方=MP；认领方=本地 orchestrator（主，10 秒轮询）+ 云端 routine（兜底，10 分钟轮询，仅本地下线时接管）。
- **红线**：对外提交（报税上传、对外邮件）与不可复原动作一律 GATE_CONFIRM 请示，绝不代拍板。
- **汇报纪律（移动端优先）**：用户在手机上看汇报，**只说结果，不播报过程**。派单/完成时的对用户输出固定三段、总长 ≤8 行：①一句话结果（干了什么/到哪一步）；②【待你决策】（没有就省略此段）；③一句话下一步。禁止向用户罗列任务 ID、内部字段表、链路检查清单、文件路径明细——这些写进滴答任务卡和 git 记录即可，用户要看细节会自己问。执行中不逐步汇报，只在完成、卡住、需决策三个时点开口。
...
| `routing-table.md` | 全局路由表：意图 → 工作区 → **代号** → 入口，含消歧规则、代号对照与直连白名单 |
| `dida-board-contract.md` | **滴答看板契约**（单一权威）：project/列 id、封闭标签词汇表、建卡模板、状态机、决策回填、双认领方约定 |
