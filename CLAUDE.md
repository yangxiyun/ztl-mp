...
1. **识别**：从用户指令中抽取〈意图 / 客户 / 期间 / 涉及文件〉。表述不明确 → 先问清（主对话可用 AskUserQuestion；MP 以 subagent 形态运行时按 AGENT-RETURN v1 以 MISSING_INPUT 一次性带齐上报）。
...
- **token 分级**：简单单点任务（开一张发票、查一个汇率）直连单 skill，禁止扇出；只有跨多 skill 的复合任务（整月做账、全套申报、深度复核）才走编排 agent/多子会话。多 agent 扇出成本约为单会话 15 倍，只花在高价值任务上。
...
| `MP-PLATFORM-BLUEPRINT.md` | 第二期躯干蓝图（Telegram 网关/同步盘/滴答看板/定时任务） |
