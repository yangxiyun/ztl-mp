...
> ⚠️ **角色变更预告（2026-08-09 用户终裁）**：MP 驾驶舱（symphony `dashboard/`，编排总线版）上线后，**滴答降为只读镜像**——驾驶舱本地任务库成为任务单一状态源，直接 spawn Claude Code 执行；滴答只收异步同步的简化状态卡（无 `agent-*` 执行标签）。本契约在过渡期仍对两条老通道有效：①claude.ai App MP Project 建卡（异地备用）②orchestrator 滴答轮询认领。驾驶舱稳定后本文件降级为镜像格式说明。
...
* 类别：<四选一：营销传播｜销售前期｜项目执行｜财务人力>（驾驶舱统计用，封闭集）
* 工作区：<代号，与 ws-* 标签后缀一致>
* 入口：<@agent名 或 skill名>
...
**`* 类别：` 行**（2026-08-09 驾驶舱扩展）：四值封闭——`营销传播` / `销售前期` / `项目执行` / `财务人力`。MP 建卡时按意图判定（大致映射：content 写作/公众号→营销传播；mgmt 商机/报价→销售前期；bps 做账/报税/客户交付→项目执行；ZTL 自账/HR/内部管理→财务人力）。旧卡无此行时，消费方按 `ws-*` 代号兜底映射，不报错。

### 3.1 `## 产出` 段（执行侧完成时回填，2026-08-09 驾驶舱扩展）

执行侧任务 DONE 交付时，在卡片 content **追加**（不覆盖原任务信息）：

```markdown
## 产出

* <产出物名称>：<WorkDrive 共享链接或 wd: 路径记号>
* <可多行，每个产出物一行>
```

- 链接优先用 WorkDrive **共享链接**（`createExternalShareLink` / TrueSync 右键分享），驾驶舱与手机端可一键直达；拿不到共享链接才退而写 `wd:` 路径记号。
- 无文件产出的任务（纯查询/纯回复）写 `* （无文件产出，结果见评论回报）`。
...
⚠️ **单机铁律（2026-08-16 用户裁定）**：同一时刻**只允许一台机器**运行本地 orchestrator——当前指定为**办公室 Windows 主机（常开）**，所有转发任务由它承接。认领判据（§7）没有机器维度，第二台机器同时跑会互相抢卡且 `recover()` 只避让 `agent-cloud`、不避让另一台本地机，无任何防护。换机或加机前必须先停掉现有实例。
